...
with open('PCIP.json') as PCIP_file:
    PCIP_MAP = json.load(PCIP_file)
...
def parse(filename):
    ...
    tables['degree'] = pd.DataFrame(columns=['UNITID','PCT','DEGREE'])
    for PCIP in PCIPs:
        df = raw_df.loc[:,['UNITID', PCIP]]
        df['DEGREE'] = PCIP_MAP[PCIP]
        df.columns = ['UNITID','PCT','DEGREE']
        tables['degree'] = tables['degree'].append(df)
...
tables['degree'].to_csv('output' + filename[-10:-7] + '/degree.csv', index=False)
